import { ButtonInteraction, ContainerBuilder, TextDisplayBuilder, SeparatorBuilder, SeparatorSpacingSize } from "discord.js";
import { getUserLang } from "../i18n";
import { buildReply, ACCENT } from "../utils/cv2";

const T: Record<string, Record<string, { title: string; desc: string }>> = {
  bienvenue: {
    fr: {
...
export async function handleTranslateButton(interaction: ButtonInteraction) {
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(new TextDisplayBuilder().setContent(`## ${content.title}`))
    .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
    .addTextDisplayComponents(new TextDisplayBuilder().setContent(content.desc))
    .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
    .addTextDisplayComponents(new TextDisplayBuilder().setContent(`-# 🌐 ${lang.toUpperCase()} • YouRazz Staff`));
